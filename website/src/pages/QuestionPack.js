...
function QuestionPack() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [testSetsResponse, categoriesResponse] = await Promise.all([
          getQuestionPacks({ _forceRefresh: Date.now() }, true),
          getCategories()
        ]);
        ...
        if (testSetsResponse.data) {
          // If it's a direct array (not wrapped in an object)
          if (Array.isArray(testSetsResponse.data)) {
            fetchedPacks = testSetsResponse.data;
          }
          // If it's a paginated response with 'results' array
          else if (testSetsResponse.data.results && Array.isArray(testSetsResponse.data.results)) {
  ...
  useEffect(() => {
    ...
    const fetchFilteredData = async () => {
      ...
      try {
        ...
        const response = await getQuestionPacks(filters, true);
        ...
        if (response.data) {
          ...
          // If it's a paginated response with 'results' array
            ...
            console.warn("Unexpected API filtered response structure:", response.data);
            filteredData = [];
          }
        }
  ...
  const handleDelete = async (e) => {
    ...
    if (deleteConfirm.packId) {
      ...
      try {
        ...
        const freshDataResponse = await getQuestionPacks({ _forceRefresh: Date.now() }, true);
        ...
        if (freshDataResponse.data) {
          ...
          // If it's a paginated response with 'results' array
          else if (freshDataResponse.data.results && Array.isArray(freshDataResponse.data.results)) {
            updatedPacks = freshDataResponse.data.results;
          } else {
            console.warn("Unexpected API response structure:", freshDataResponse.data);
            updatedPacks = [];
          }
        }
        ...
        if (filters.category || filters.difficulty || filters.search) {
          ...
          if (filteredResponse.data) {
            ...
            // If it's a paginated response with 'results' array
            else if (filteredResponse.data.results && Array.isArray(filteredResponse.data.results)) {
              newFilteredPacks = filteredResponse.data.results;
            } else {
              console.warn("Unexpected API filtered response structure:", filteredResponse.data);
              newFilteredPacks = [];
            }
          }
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const testSetsResponse = await getQuestionPacks({ _forceRefresh: Date.now() }, true);
