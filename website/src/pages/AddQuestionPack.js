...
function AddQuestionPack() { // Removed props, use hooks instead
  ...
  // Debug logging to check if edit mode is detected
  console.log('Edit mode detected:', { packId, isEditing });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (isEditing) {
          console.log('Fetching data for pack ID:', packId);
          
          try {
            const packResponse = await getQuestionPack(packId);
            console.log('Fetched pack data:', packResponse.data);
            
            const existingPackData = packResponse.data;
            
            // Check if we got valid pack data
            if (!existingPackData) {
              throw new Error('Could not load question pack data. The pack ID may be invalid.');
            }
            
            // Set pack details from API response
            setPackDetails({
              ...
              category: existingPackData.category?.id || existingPackData.category_id || '',
            });
            
            // Fetch questions for this pack
            try {
              console.log('Fetching questions for pack ID:', packId);
              const questionsResponse = await getQuestions(packId);
              console.log('Fetched questions data:', questionsResponse.data);
              ...
              if (questionsResponse.data) {
                ...
                if (Array.isArray(questionsResponse.data)) {
                  existingQuestions = questionsResponse.data;
                } else if (questionsResponse.data.results && Array.isArray(questionsResponse.data.results)) {
                  existingQuestions = questionsResponse.data.results;
                } else if (typeof questionsResponse.data === 'object' && questionsResponse.data.id) {
                  // If it's a single question object, wrap it in an array
                  existingQuestions = [questionsResponse.data];
                } else {
                  console.warn('Unexpected questions data format:', questionsResponse.data);
                }
              }
              
              console.log('Processed questions data:', existingQuestions);
              
              // For each question, fetch its options
              try {
                console.log('Fetching options for each question');
                const questionsWithOptions = await Promise.all(
                  existingQuestions.map(async (question) => {
                    try {
                      console.log('Fetching options for question ID:', question.id);
                      const optionsResponse = await getOptions(question.id);
                      console.log('Options for question:', optionsResponse.data);
                      ...
                      if (optionsResponse.data) {
                        ...
                        if (Array.isArray(optionsResponse.data)) {
                          questionOptions = optionsResponse.data;
                        } else if (optionsResponse.data.results && Array.isArray(optionsResponse.data.results)) {
                          questionOptions = optionsResponse.data.results;
                        } else if (typeof optionsResponse.data === 'object' && optionsResponse.data.id) {
                          // If it's a single option object, wrap it in an array
                          questionOptions = [optionsResponse.data];
                        } else {
                          console.warn('Unexpected options data format:', optionsResponse.data);
                        }
                      }
                      ...
                      return {
                        ...
                        dbSaved: true,  // Flag to indicate this is from the database
                      ...
                      console.error(`Error fetching options for question ID ${question.id}:`, optionError);
                      // Return the question without options as fallback
                      return {
                        id: question.id,
                        text: question.content,
                        explanation: question.explanation || '',
                        dbSaved: true,
                        options: [] // Empty options array as fallback
                      };
                    }
                  })
                );
                
                console.log('All questions with options:', questionsWithOptions);
                setQuestions(questionsWithOptions);
              } catch (optionsError) {
                console.error('Error fetching options for questions:', optionsError);
                showMessage('warning', 'Could not load all question options. Some data may be missing.');
              }
            } catch (questionsError) {
              console.error('Error fetching questions:', questionsError);
              showMessage('warning', 'Could not load questions. You may need to add them again.');
            }
          } catch (packError) {
            console.error('Error fetching pack data:', packError);
            throw new Error('Could not load the question pack. It may have been deleted or is inaccessible.');
          }
        } else {
    ...
    try {
      ...
      for (const question of questions) {
        ...
        if (isEditing && question.dbSaved) {
          try {
            console.log(`Checking for removed options for question ID ${question.id}`);
            
            // Get current options from database
            const optionsResponse = await getOptions(question.id);
            console.log('Current options in database:', optionsResponse.data);
            
            // Ensure we have an array of database options
            let dbOptions = [];
            if (optionsResponse.data) {
              if (Array.isArray(optionsResponse.data)) {
                dbOptions = optionsResponse.data;
              } else if (optionsResponse.data.results && Array.isArray(optionsResponse.data.results)) {
                dbOptions = optionsResponse.data.results;
              } else if (typeof optionsResponse.data === 'object' && optionsResponse.data.id) {
                dbOptions = [optionsResponse.data];
              }
            }
            
            // Get IDs of options currently in our form for this question
            const currentOptionIds = question.options
              .filter(o => o.dbSaved)
              .map(o => o.id);
              
            console.log('Current option IDs in form:', currentOptionIds);
            ...
            for (const option of optionsToDelete) {
              console.log(`Deleting option ID ${option.id}`);
              await deleteOption(option.id);
            }
          } catch (optionDeleteError) {
            console.error(`Error handling option deletion for question ${question.id}:`, optionDeleteError);
            // Continue with next question despite option deletion error
          }
        }
      }
      ...
      if (isEditing) {
        try {
          ...
          const questionsResponse = await getQuestions(packId);
          console.log('Current questions in database:', questionsResponse.data);
          
          // Ensure we have an array of database questions
          let dbQuestions = [];
          if (questionsResponse.data) {
            if (Array.isArray(questionsResponse.data)) {
              dbQuestions = questionsResponse.data;
            } else if (questionsResponse.data.results && Array.isArray(questionsResponse.data.results)) {
              dbQuestions = questionsResponse.data.results;
            } else if (typeof questionsResponse.data === 'object' && questionsResponse.data.id) {
              dbQuestions = [questionsResponse.data];
            }
          }
          
          // Get IDs of questions currently in our form
          const currentQuestionIds = questions
            .filter(q => q.dbSaved)
            .map(q => q.id);
            
          console.log('Current question IDs in form:', currentQuestionIds);
          ...
          if (questionsToDelete.length > 0) {
            showMessage('info', `Deleted ${questionsToDelete.length} removed question(s)`);
          }
        } catch (deleteError) {
          console.error('Error handling question deletion:', deleteError);
          showMessage('warning', 'Could not delete some removed questions');
        }
      }
